...
## Un solo repo, dos despliegues automáticos

No necesitas repos separados. Subes esta carpeta completa a un solo repo de GitHub y:

- **Cloudflare Pages** queda conectado directo al repo, mirando solo la carpeta `frontend/` (se configura una vez, ver paso 3).
- **El Worker** se despliega solo en cada push gracias al archivo `.github/workflows/deploy-worker.yml` que ya está incluido.

Para que el Worker se despliegue solo, necesitas un token de Cloudflare guardado como secreto en GitHub:

1. Ve a **Cloudflare dashboard → My Profile → API Tokens → Create Token** → usa la plantilla "Edit Cloudflare Workers".
2. En tu repo de GitHub: **Settings → Secrets and variables → Actions → New repository secret**.
3. Nombre: `CLOUDFLARE_API_TOKEN`, valor: el token que copiaste.

A partir de ahí, cada vez que hagas `git push` a `main` con cambios en `worker/`, el Worker se actualiza solo. No vuelves a correr `wrangler deploy` a mano.
